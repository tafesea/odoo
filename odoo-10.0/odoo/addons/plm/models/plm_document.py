...
class AttachmentExtension(models.Model):
    ...
    def check(self, mode, values=None):
        ...
        if self:
            toExec = 'SELECT res_model, res_id, create_uid, public FROM %s WHERE id IN ' % (self._table)
            toExec += '%s'
            self._cr.execute(toExec, [tuple(self.ids)])
            ...
            if objDoc.type == 'binary':
                if objDoc.store_fname:
                    ...
                    if os.path.exists(filestore):
                        objDoc.datas = open(filestore, 'rb').read().encode('base64')
                else:
    ...
    def write(self, vals):
        ...
        if check:
            for customObject in self:
                if customObject.state in checkState and self.env.uid != SUPERUSER_ID:
...
#   Overridden methods for this entity
    ...
    def GetRelatedDocs(self, default=None):
        ...
        for document in self.browse(read_docs):
            ...
            if not preview:
                preview=False
            related_documents.append([document.id, document.name, preview, document.revisionid, document.description])
    ...
    def read(self, fields=None, load='_classic_read'):
        return super(PlmDocument, self).read(fields, load=load)
