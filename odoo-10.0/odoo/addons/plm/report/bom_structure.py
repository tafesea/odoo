...
def get_bom_report(myObject, recursion=False, flat=False, leaf=False, level=1, summarize=False):
    ...
    def getOutLineInfos(bom_line_id, product_tmpl_id, prodQty):
        res = {}
        res['row_bom_line'] = bom_line_id
        res['name'] = product_tmpl_id.engineering_code
        res['item'] = bom_line_id.itemnum
        res['pname'] = product_tmpl_id.engineering_code
        res['pdesc'] = _(product_tmpl_id.description)
        res['pcode'] = bom_line_id.product_id.default_code
        res['previ'] = product_tmpl_id.engineering_revision
        res['pqty'] = prodQty
        res['uname'] = bom_line_id.product_uom_id.name
        res['pweight'] = product_tmpl_id.weight
        res['code'] = bom_line_id.product_id.default_code
        res['level'] = level
        res['prodBrws'] = bom_line_id.product_id
        res['prodTmplBrws'] = product_tmpl_id
        res['lineBrws'] = bom_line_id
        return res
