...
class SubTaskMaster(models.Model):
    ...
    # def cheklist_progress(self):
    #     for rec in self:
    #         total_len = self.env['task.checklist'].search_count([])
    #     check_list_len = len(rec.unit_amount)
    #     if total_len != 0:
    #         rec.checlist_progress = (check_list_len * 100 / total_len)
    #
    # unit_amount = fields.Manay2many('task.checklist', string='Check List')
    # checklist_progress = fields.Float(compute=cheklist_progress, String='Progress', store=True, recompute=True,
    #                                   default=0.0)
    ...
    def _hours_get(self):
        for task in self.sorted(key='id', reverse=True):
            ...
            task.effective_hours = sum(task.sub_task_ids.mapped('unit_amount'))
    ...
    remaining_hours = fields.Float(compute='_hours_get', store=True, string='Target Progress',
                                       help="Total remaining time, can be re-estimated periodically by the assignee of the task.")
    progress = fields.Float(string='Target Progress in Percent', group_operator="avg")
    parent_id = fields.Many2one('project.sub_task', string='Component')
    child_ids = fields.One2many('project.sub_task', 'parent_id', string="Sub-tasks")
    ...
    task_ref = fields.Many2one('project.task', string='KPI', required=1,
    ...
    task_report = fields.Html(sting='Task Report')
    tag_ids = fields.Many2many('project.tags', string='Tags', oldname='categ_ids')
    planned_hours = fields.Float(string='Target',
    ...
    sub_task_ids = fields.One2many('sub.task.progress', 'progress_id', 'Timesheet')
    ...
    description = fields.Html(String='Report Detail')
    ...
    sub_timesheet_ids = fields.One2many('account.analytic.line', 'sub_task_id', 'Timesheets')
...
class SubTaskProgress(models.Model):
    _name = "sub.task.progress"

    progress_id = fields.Many2one('project.sub_task', 'Sub Task')
    description = fields.Char('Description')
    unit_amount = fields.Float('Unit Amount', required=True)
    date_of_progress = fields.Datetime('Date Time')

class TaskCheclist(models.Model):
