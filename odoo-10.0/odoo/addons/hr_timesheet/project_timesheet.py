...
class Task(models.Model):
    ...
    remaining_hours = fields.Float(compute='_hours_get', store=True, string='Remain Target Progress', help="Total remaining time, can be re-estimated periodically by the assignee of the task.")
    effective_hours = fields.Float(compute='_hours_get', store=True, string='Total Target Progress', help="Computed using the sum of the task work done.")
    ...
    sub_task_timesheet_ids = fields.One2many('account.analytic.line', 'sub_task_id', 'Timesheets')

    parent_id = fields.Many2one('project.task', string='Component')
