...
class PlmComponent(models.Model):
    ...
    def on_change_engineering_code(self):
        if self.engineering_code:
            filter_to_apply = [
                ('engineering_code', '=', self.engineering_code),
                ('engineering_revision', '=', self.engineering_revision),
                               ]
            if not self.search_count(filter_to_apply):
                filter_to_apply.append(('active', '=', False))
                count = self.search_count(filter_to_apply)
                if count > 0:
                    raise UserError(_('Component with Part Number "%r" and revision "%r" already exists. N.B. Component is archived!' % (self.engineering_code, self.engineering_revision)))
            else:
                raise UserError(_('Component with Part Number "%r" and revision "%r" already exists.' % (self.engineering_code, self.engineering_revision)))
...
#   Internal methods
    ...
    def action_view_bom(self):
        return self.open_boms()

    def _compute_bom_count(self):
        for product in self:
            product.bom_count = product.product_tmpl_id.bom_count
    ...
    def action_release(self):
        ...
        """
        for compObj in self:
            if compObj.state =='released':
                logging.warning("Try to release %r that is released" % compObj.id)
                continue
            ...
            for productBrw in allProdObjs:
                ...
                if old_revision:
                    ...
                    old_revision.wf_message_post(body=_('Status moved to: %s by %s.' % (USEDIC_STATES[defaults['state']], self.env.user.name)))
            defaults['engineering_writable'] = False
            ...
            if (objId):
                self.browse(product_ids).wf_message_post(body=_('Status moved to: %s by %s.' % (USEDIC_STATES[defaults['state']], self.env.user.name)))
            return objId
        return False
    ...
    def _action_to_perform(self, status, action, docaction, defaults=[], excludeStatuses=[], includeStatuses=[]):
        ...
        if objId:
            self.browse(allIDs).wf_message_post(body=_('Status moved to: %s by %s.' % (USEDIC_STATES[defaults['state']], self.env.user.name)))
        return objId
...
Please try to contact OmniaSolutions to solve this error, or install Plm Sale Fix module to solve the problem.''' % (ex)
    ...
    def NewRevision(self):
        ...
        for tmpObject in self:
            ...
            for oldObject in self.browse(latestIDs[-1]):
                ...
                oldObject.wf_message_post(body=_('Status moved to: %s by %s.' % (USEDIC_STATES[oldProdVals['state']], self.env.user.name)))
    ...
    def getBomRowCad(self, bomLineBrowse):
        ...
        return [bomLineBrowse.itemnum,
                ...
                self._translate(emptyStringIfFalse(bomLineBrowse.product_id.with_context(ctx).description), 'english'),
